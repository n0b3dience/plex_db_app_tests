...
from .helpers import pms_image_proxy, info_page, library_page, user_page
...
class Tautulli:
    ...
    def __init__(self, host=None, port=None, apikey=None,
        ...
        self.url = '{0}/api/v2'.format(self.base_url)
    ...
    def get_page(self, endpoint, *args, **kwargs):
        """
        Gets a full link for an image, info page, library page or user page.

        Good for getting a full URL to a Tautulli page (user, show, movie, 
        episode, etc.) or image.

        Usage example:
            t.get_page('pms_image_proxy',
                       img='/library/metadata/2414/thumb/1637919009)
        
        Alternate usage example:
            thumb = t.get_metadata(rating_key=2414)['response']['data']['thumb']
            t.get_page('pms_image_proxy', img=thumb)

        Examples will return:
            'http://localhost:8181/pms_image_proxy?img=%2Flibrary
            %2Fmetadata%2F2414%2Fthumb%2F1637919009'

        Returns:
            [URL]

        'pms_image_proxy' kwargs:
            img, rating_key, width, height, opacity, background, blur, 
            img_format, fallback, refresh, clip

        'info' kwargs:
            rating_key, guid, history, live
        ...
        url_path = '{0}?{1}'.format(endpoint, urlencode(params))

        return '{0}/{1}'.format(self.base_url, url_path)
